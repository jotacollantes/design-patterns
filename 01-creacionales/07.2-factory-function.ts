...
function createLogger(level: LogLevel) {
  ...
  return function (message: string) {
    const log = {
      ...
      warning: `%c[WARNING: ${formatDate(new Date())}] ${message}`,
...
const clg = (message: string, color: keyof typeof COLORS) => {
  console.log(message, COLORS[color]);
}
// Ejemplo de uso
function main() {
  ...
  clg(infoLogger("Aplicación iniciada correctamente."),'green');
  clg(warnLogger("El uso de memoria está alto."),'yellow');
  clg(errorLogger("Error de conexión a la base de datos."),'red');
}

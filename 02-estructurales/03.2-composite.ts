...
class MenuCategory implements MenuComponent {
  ...
  add(item: MenuComponent | MenuComponent[]): void {
    //verifico si item es un arreglo
    // tambien puede ser if(item instanceof Array)
...
function main() {
  ...
  const bebidas = new MenuCategory("Bebidas");
  const bebidasCalientes = new MenuCategory("Bebidas Calientes");
  const bebidasFrias = new MenuCategory("Bebidas Frias");

  bebidasFrias.add(soda);
  bebidasCalientes.add(coffee);
  bebidasCalientes.add(te);
  bebidas.add([bebidasCalientes, bebidasFrias]);
  ...
  mainMenu.add([appetizers, bebidas, desserts, mainCourse]);
